-- ---------------------------------------------------------------------------
-- Deliberate reportability change (not a bug fix): a payment made against a
-- saved Stripe customer/card frequently carries no email or phone on the
-- charge, checkout session, invoice, or payment method at all -- only the
-- Stripe Customer object does, because Stripe never copies a customer's
-- saved contact info onto a later charge. b2c_payment_decision_reasons was
-- given only the charge-level `effective_customer_email`, so such a payment
-- stayed blocked by "missing_customer_email" even when the same email was
-- already fetched and shown as a "not yet verified" display fallback --
-- requiring a manual local correction or Finance exception for data PLAYBOOK
-- already had.
--
-- Decision (confirmed with the product owner, after reviewing the real
-- blast radius: 357 succeeded payments / $71,320.76 currently blocked only
-- for this reason as of 2026-09-17): a Stripe-sourced email from ANY
-- captured source -- charge, checkout, invoice, payment method, or customer
-- profile -- now satisfies "has an email" for reportability. The
-- underlying source data is never fabricated or mutated: `customer_email`
-- on `b2c_payments` stays exactly what the charge itself provided (or
-- null), and an Admin can still correct/override it manually at any time
-- through the existing "Other details"/email UI -- that stays fully
-- optional now, not mandatory. A payment with no email anywhere, including
-- the profile, is completely unaffected: it still blocks and still needs a
-- verified email or an audited Finance exception exactly as before.
--
-- Mechanically: `b2c_payment_decision_reasons` is fed `display_customer_email`
-- (the same coalesce already used for what the Ledger and drawer show)
-- instead of `effective_customer_email` (charge-level only, ignoring the
-- Stripe payment-method/customer-profile fallback), for both the payment and
-- linked-refund decision calls. The Ledger's separate "Missing customer
-- email" issue badge and its open_review_flags entry -- both driven by the
-- stored review_flags row from ingestion, independent of the decision
-- function -- are given the same "no display email available" gate so all
-- three signals (blocking, issue badge, open flag) agree. Nothing else in
-- this function changes (diffed against the prior definition in
-- 20270101001000_b2c_missing_email_fallback_display.sql to confirm this).
--
-- This DOES change Reportable Cash: any payment whose only blocking reason
-- was a missing email, and which has a Stripe payment-method or
-- customer-profile email on file, becomes reportable the moment this
-- migration is applied -- no Admin action required for that reason alone.
-- ---------------------------------------------------------------------------
...
as $$
  ...
  payment_base as (
    select
      ...
      (
        coalesce(payment_flags.has_missing_email, false)
        and coalesce(
  ...
  payment_decisions as (
    select payment_base.*,
      public.b2c_payment_decision_reasons(
        ...
        payment_base.display_customer_email,
  ...
  refund_rows as (
    ...
    cross join lateral (
      select public.b2c_payment_decision_reasons(
        'succeeded',
        payment.display_customer_email,
