...
## 2026-08-24 supersession note — provider product mapping and review

The mapping and `Unmapped product` requirements above are retained verbatim as
historical source material. The approved
[`optional provider product category design`](superpowers/specs/2026-08-24-optional-provider-product-category-design.md)
supersedes them for the clean rebuild: Stripe and Tap descriptions are source
evidence and the visible product label; category/tier are optional local
metadata; and `unmapped` remains only the internal duplicate-fingerprint
value, not a reportability gate.

Product-mapping tables, functions, classifications, corrections, and historical
`unmapped_product` flags remain read-only audit history. Mapping is absent from
the B2C Work queue, Ledger issues, generic Review Queue, and shared drawer.
Manual bank-transfer and Finance Tracker category requirements are unchanged.
The Finance exception no longer requires category, but retains every other
approved safeguard.
