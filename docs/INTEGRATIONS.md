...
- An Admin may also resolve or dismiss any B2C review item with a required note. Closing an `unmapped_product` or `Missing customer email` task does not manufacture reportability: the financial calculation still requires an actual mapping and direct source email.
