...
`b2c_payments` has a partial unique provider-ID index on `(source_system, provider_transaction_id)` and a 64-character deterministic content fingerprint index. Its direct source name, email, phone, and provider description fields are nullable so incomplete provider records remain traceable rather than disappearing. `customer_name` and `customer_phone` are stored only when the financial provider supplied them directly; absent values remain absent rather than being inferred. The provider description is source evidence and the visible product label; it is never inferred from, or used instead of, category. A missing source email creates an isolated provider-ID fingerprint and an open review flag. A verified Admin local email/category correction is a separate overlay, closes only the corresponding missing-data flag, and performs the same 48-hour duplicate check before it can be counted. Later ingestion normalizes `lower(trim(email))`, a canonical six-decimal amount, category identifier (including stable `unmapped`), and Bahrain business date before comparing matching fingerprints across the preceding 48 hours. `unmapped` is not a provider-payment reportability gate. A resolution note alone cannot make a payment reportable.
