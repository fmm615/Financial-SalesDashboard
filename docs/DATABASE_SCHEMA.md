...
| B2C | `customers`, `products`, `product_mappings`, `b2c_payments`, `b2c_payment_local_overrides`, `b2c_refunds`, `b2c_payment_duplicate_groups`, `b2c_payment_duplicate_group_members` | B2C records only. Stripe and Tap source checks prevent a Stripe record becoming B2B. The Payment Tracker Excel-workbook system (staging, cross-tab duplicate grouping, lineage, and iOS/bank-transfer posting) is not part of the schema (`20270101000200_b2c_foundation.sql`, the B2C domain migration, deliberately excludes it entirely); historical `finance_tracker`-sourced `b2c_payments` rows remain as immutable ledger history. |
...
| Reports | `report_jobs`, `reports`, `report_files`, `report_delivery_attempts` | Persistent job/archive structures plus an isolated draft-fixture PDF/CSV workflow. The protected worker processes only `draft_fixture` jobs, records snapshot readiness/version, and marks stale draft jobs after 15 minutes as failed. The `report_delivery_attempts` table is reserved for a future approved integration: the current disabled provider creates no rows, recipients, HTTP requests, or email credentials. |
...
`financial_corrections` captures target record, before value, after value, reason, Admin actor, and time. It does not overwrite the original record. The audit trigger independently captures the individual `auth.uid()` with a before/after snapshot for every allowed Admin write. `b2c_payment_local_overrides` stores verified Admin-only local values for one source payment (name, email, phone, category, or tier), leaving the provider row untouched. Product mappings and their related classification corrections are retained historical audit data: `20270101000200_b2c_foundation.sql` preserves their approved read access while revoking live mapping-function execution and mapping-table writes.
...
`b2c_payments` has a partial unique provider-ID index on `(source_system, provider_transaction_id)` and a 64-character deterministic content fingerprint index. Its direct nullable source fields are name, email, and phone, so incomplete provider records remain traceable rather than disappearing. Provider descriptions are retained in `b2c_payments.source_metadata` JSON and, for Stripe, in selected stored Stripe evidence; they are source evidence and the visible product label, never inferred from or used instead of category. `customer_name` and `customer_phone` are stored only when the financial provider supplied them directly; absent values remain absent rather than being inferred. A missing source email creates an isolated provider-ID fingerprint and an open review flag. A verified Admin local email correction is a separate overlay that closes its matching missing-email flag and must still pass the same 48-hour duplicate check before it can be counted. Category remains optional per-record metadata and is not a reportability gate; retired historical `unmapped_product` flags have no live mapping workflow. Later ingestion normalizes `lower(trim(email))`, a canonical six-decimal amount, category identifier (including stable `unmapped`), and Bahrain business date before comparing matching fingerprints across the preceding 48 hours. A resolution note alone cannot make a payment reportable.
