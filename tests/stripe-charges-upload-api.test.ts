...
vi.mock("@/server/services/b2c-provider-evidence-reconciliation", () => ({ linkB2cProviderEvidence: vi.fn() }));
...
const linkB2cProviderEvidenceMock = vi.mocked(linkB2cProviderEvidence);
...
describe("Stripe Charges upload boundary", () => {
  beforeEach(() => { vi.resetAllMocks(); parseStripeChargesCsvMock.mockReturnValue(parsedCharges); linkB2cProviderEvidenceMock.mockResolvedValue({ exactMatches: [], mismatches: [], unmatchedEvidence: [] }); });
  ...
  it("reconciles exact Stripe provider-ID evidence after a successful import", async () => {
    ...
    expect(linkB2cProviderEvidenceMock).toHaveBeenCalledWith(client, { importId: "22222222-2222-4222-8222-222222222222", provider: "stripe" });
  });
  ...
  it("never fails a successful import when evidence reconciliation itself fails", async () => {
    ...
    createServerClientMock.mockResolvedValue(client as never);
    getApprovedRoleMock.mockResolvedValue("admin");
    linkB2cProviderEvidenceMock.mockRejectedValue(new Error("evidence link failure"));
