...
describe("SupabaseB2cLedgerRepository", () => {
  it("requests limit plus one identities and hydrates only the returned page IDs", async () => {
    ...
    const rpc = vi.fn(async (name: string, args: Record<string, unknown>) => {
      ...
      if (name === "get_b2c_ledger_metadata") {
        return { data: { total_count: 3, sources: ["Stripe"], categories: ["membership"], issues: [], foreign_currency_count: 0 }, error: null };
      }
  ...
  it("returns an empty bounded page without issuing a hydration RPC", async () => {
    const rpc = vi.fn(async (name: string) => {
      ...
      if (name === "get_b2c_ledger_metadata") {
        return { data: { total_count: 0, sources: [], categories: [], issues: [], foreign_currency_count: 0 }, error: null };
      }
  ...
  it("preserves the legacy compact display for a foreign source amount", async () => {
    ...
    const rpc = vi.fn(async (name: string) => {
      ...
      if (name === "get_b2c_ledger_metadata") return { data: { total_count: 1, sources: ["Stripe"], categories: ["membership"], issues: [], foreign_currency_count: 1 }, error: null };
