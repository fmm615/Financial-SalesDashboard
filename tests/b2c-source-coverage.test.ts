...
describe("B2C source coverage", () => {
  it("withholds financial totals before historical Stripe data is fully loaded", () => {
    const coverage = resolveB2cSourceCoverage({
      historicalBackfill: { status: "processing", recordsFailed: 0, completedAt: null },
  ...
  it("withholds financial totals when the completed backfill has source failures", () => {
    const coverage = resolveB2cSourceCoverage({
      historicalBackfill: { status: "completed", recordsFailed: 2, completedAt: "2026-08-05T08:00:00.000Z" },
      latestReconciliation: null,
    });
  ...
  it("makes financial totals available after a clean historical import and advances the as-of timestamp", () => {
    const coverage = resolveB2cSourceCoverage({
      historicalBackfill: { status: "completed", recordsFailed: 0, completedAt: "2026-08-05T08:00:00.000Z" },
      latestReconciliation: { status: "completed", requestedRangeEnd: "2026-08-05T12:00:00.000Z", completedAt: "2026-08-05T12:01:00.000Z" },
    });
