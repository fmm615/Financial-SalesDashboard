...
export async function POST(_: NextRequest, context: { params: Promise<{ jobId: string }> }) {
  ...
  const { data: job, error: readError } = await client.from("report_jobs").select("status,retry_count").eq("id", parsed.data.jobId).maybeSingle();
  if (readError || !job || job.status !== "failed") return NextResponse.json({ error: "Only failed report jobs can be retried." }, { status: 422 });
  const { error } = await client.from("report_jobs").update({
    status: "pending", retry_count: job.retry_count + 1, failed_at: null, safe_error_summary: null,
