...
/** Records an append-only Finance conversion locally. It never calls Tap or Stripe. */
export async function POST(request: NextRequest, { params }: { params: Promise<{ paymentId: string }> }) {
  ...
  const { data, error } = await client.rpc("record_b2c_payment_fx_conversion", {
  ...
  if (error) return NextResponse.json({ error: "The Finance USD conversion could not be saved." }, { status: 422 });
