...
function DuplicateDecision({ group, onSaved }: { group: DuplicateGroup; onSaved: () => Promise<void> }) {
  ...
  return <article className="border-b border-line py-5 first:pt-0 last:border-b-0 last:pb-0">
    <div><p className="font-medium text-ink">Possible duplicate HubSpot deals</p><p className="mt-1 text-sm text-slate-600">Flagged {formatDate(group.flaggedAt)} · compare the source IDs before deciding.</p></div>
...
/** Admin-only choice for exact matching HubSpot deals. No record is deleted. */
export function HubSpotDuplicateReview() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(body.error ?? "Could not load HubSpot duplicate candidates.");
      setReview(body);
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : "Could not load HubSpot duplicate candidates.");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  return <div className="mt-4"><SectionCard title="Possible duplicate HubSpot deals" description="Exact candidates are paused from financial totals until an Admin chooses to keep both or keep one. The decision is audited and never writes to HubSpot.">
    {review?.groups.length ? <div>{review.groups.map((group) => <DuplicateDecision key={group.id} group={group} onSaved={load} />)}</div> : <p className="text-sm text-slate-600">No possible HubSpot duplicates are waiting for review.</p>}
