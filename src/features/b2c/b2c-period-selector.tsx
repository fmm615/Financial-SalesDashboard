...
function monthOptions(selectedMonth: string): { value: string; label: string }[] {
  ...
  for (let year = lastYear; year >= firstYear; year -= 1) for (let month = 11; month >= 0; month -= 1) {
...
export function B2cPeriodSelector({ month }: { month: string }) {
  ...
  return <div className="relative inline-flex items-center rounded-pill border border-border bg-surface text-sm font-medium text-text-secondary shadow-card hover:border-brand-accent/30 hover:text-text-primary">
    ...
    <select aria-label="B2C financial reporting month" value={month} onChange={(event) => selectMonth(event.target.value)} className="cursor-pointer appearance-none bg-transparent py-2 pl-2 pr-8 text-sm font-medium outline-none">
