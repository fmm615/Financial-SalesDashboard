...
export function B2cPeriodSelector({ month }: { month: string }) {
  ...
  return <div className="relative inline-flex items-center rounded-pill border border-border bg-surface text-sm font-medium text-text-secondary shadow-card hover:border-brand-accent/30 hover:text-text-primary">
    ...
    <select aria-label="B2C reporting period" value={month} onChange={(event) => selectMonth(event.target.value)} className="cursor-pointer appearance-none bg-transparent py-2 pl-2 pr-8 text-sm font-medium outline-none">
