...
/** Tap statement evidence upload as a state machine: select, `Preview`, then one final import action. */
export function B2cTapStatementUpload({ onImported }: { onImported: () => void }) {
  const [file, setFile] = useState<File | null>(null);
  const [preview, setPreview] = useState<TapStatementPreview | null>(null);
  ...
  async function requestPreview() {
    ...
    try {
      ...
      if (!response.ok || !payload || typeof payload !== "object" || !("preview" in payload)) {
        throw new Error(payload && typeof payload === "object" && "error" in payload && typeof payload.error === "string" ? payload.error : "The Tap statement preview could not be prepared.");
      }
      setPreview(payload.preview as TapStatementPreview);
      setStage("previewed");
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : "The Tap statement preview could not be prepared.");
      setStage("select");
    }
  }
  ...
  async function confirmImport() {
    ...
    try {
      ...
      setError(caught instanceof Error ? caught.message : "The Tap statement could not be staged.");
      setStage("previewed");
    }
  }
  ...
  return <div>
    ...
    <input
      id="tap-statement-csv" type="file" accept=".csv,text/csv" disabled={stage === "previewing" || stage === "importing"}
    ...
    <div className="mt-3 flex flex-wrap items-center gap-3">
      {stage === "select" || stage === "previewing" ? (
        ...
        <>
          <PrimaryButton onClick={() => void confirmImport()} disabled={stage === "importing"}>{stage === "importing" ? "Importing…" : "Import reviewed statement"}</PrimaryButton>
    ...
    <div className="mt-4 min-h-[3.5rem]">
      {preview && <div className="rounded-md border border-border bg-canvas p-4 text-sm text-text-secondary" role="status">
        <p className="font-medium text-text-primary">{preview.totalRows} evidence rows</p>
        <p className="mt-1">{preview.kindCounts.sale} sales · {preview.kindCounts.processing_fee} processing fees · {preview.kindCounts.fee_vat} fee VAT · {preview.kindCounts.refund} refunds · {preview.kindCounts.transfer} transfers</p>
        <p className="mt-1">{preview.kindCounts.opening_balance} opening balances · {preview.kindCounts.needs_review} need review · {preview.missingPaymentIdSales} sales missing Tap payment IDs · {preview.unparsedDates} raw dates retained</p>
      </div>}
    </div>
  </div>;
}
