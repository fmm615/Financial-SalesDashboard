...
export function B2cLedgerFilters({ filters, onChange, sources, categories, issues, shownCount, totalCount }: {
  ...
  return <div className="mb-5 rounded-input border border-border bg-surface-muted/30 p-4">
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5">
      ...
      <label>Category<select name="category" value={filters.category} onChange={update} className={inputClass}><option value="all">All categories</option>{categories.map((category) => <option key={category.value} value={category.value}>{category.label}</option>)}</select></label>
