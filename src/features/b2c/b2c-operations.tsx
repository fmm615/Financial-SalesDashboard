...
function filterRows(rows: B2cLedgerRow[], filters: typeof initialB2cLedgerFilters): B2cLedgerRow[] {
  ...
  return rows.filter((row) => {
    const searchable = [row.customerName, row.customerEmail, row.customerPhone, row.providerReference]
      .filter(Boolean)
      .join(" ")
      .toLowerCase();
...
function sortRows(rows: B2cLedgerRow[], sort: LedgerSort): B2cLedgerRow[] {
  ...
  return [...rows].sort((first, second) => {
    ...
    if (first.amountValueUsd === null) return second.amountValueUsd === null ? 0 : multiplier;
    if (second.amountValueUsd === null) return -multiplier;
  ...
  return (
    <TableHeader>
      <button
        ...
        className="inline-flex items-center gap-1 font-inherit text-left hover:text-text-primary"
        aria-label={`Sort by ${label}, currently ${directionLabel}`}
      >
        {label}
        <span aria-hidden="true" className={isCurrent ? "text-brand-accent" : "text-text-muted"}>
          {isCurrent ? (sort.direction === "ascending" ? "↑" : "↓") : "↕"}
        </span>
      </button>
    </TableHeader>
  );
...
function LedgerTable({
  ...
  return (
    <DataTable caption="B2C source ledger">
      <TableHead>
        ...
        <TableHeader>
          <span className="sr-only">Details and admin actions</span>
        </TableHeader>
      </TableHead>
          ...
          <tr key={`${row.recordType}-${row.id}`}>
            ...
            <TableCell>
              ...
              {row.customerEmailEvidenceLabel && (
                <span className="mt-0.5 block text-xs text-text-muted">{row.customerEmailEvidenceLabel}</span>
              )}
            </TableCell>
            <TableCell>
              <span>{row.customerPhone ?? "—"}</span>
              {row.customerPhoneEvidenceLabel && (
                <span className="mt-0.5 block text-xs text-text-muted">{row.customerPhoneEvidenceLabel}</span>
              )}
            </TableCell>
            <TableCell>{row.recordType}</TableCell>
            <TableCell>
              {row.date}
              {row.hasLocalCorrection && (
                <span className="mt-1 block text-xs font-medium text-brand-accent">Locally corrected</span>
              )}
            </TableCell>
            ...
            <TableCell className="space-y-1">
              ...
              {row.foreignCurrencyReview && <StatusBadge status="Needs FX review" />}
              {row.issue ? <StatusBadge status={row.issue} /> : !row.hasFinanceException && !row.foreignCurrencyReview ? "—" : null}
            </TableCell>
...
export function B2cOperations({
  ...
  const sources = useMemo(
    () => (snapshot ? [...new Set(snapshot.rows.map((row) => row.source))].sort().map((value) => ({ value, label: value })) : []),
    [snapshot],
  );
  const categories = useMemo(
    () => (snapshot ? [...new Set(snapshot.rows.map((row) => row.category))].sort().map((value) => ({ value, label: value })) : []),
    [snapshot],
  );
  const issues = useMemo(
    () => (snapshot ? [...new Set(snapshot.rows.flatMap((row) => (row.issue ? [row.issue] : [])))].sort().map((value) => ({ value, label: value })) : []),
    [snapshot],
  );
  ...
  return (
    <AppShell
      ...
      description="Stripe and other B2C payments are kept separate from B2B. Only completed, verified, non-duplicate payments contribute to totals; a Finance exception is clearly labelled and fully audited. Refunds remain separate source records."
      controls={snapshot ? <B2cPeriodSelector month={snapshot.period.month} /> : undefined}
    >
      {loadError || !snapshot ? (
        <ErrorState title="B2C data unavailable" description={loadError ?? "The B2C dashboard could not be loaded."} />
      ) : (
        <>
          <div className={`mb-4 rounded-card border p-4 ${financialTotalsAvailable ? "border-success/25 bg-success/5" : "border-warning/30 bg-warning/5"}`} role="status">
            ...
            <p className="mt-1 text-sm leading-6 text-text-secondary">
              {snapshot.sourceCoverage.description}
              {sourceAsOf ? ` Source data is current through ${sourceAsOf} Bahrain time.` : ""}
            </p>
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2 2xl:grid-cols-5">
            ...
            <MetricCard label={`Retrieved completed USD source payments · ${snapshot.period.monthLabel}`} value={snapshot.hasSourceRecords ? snapshot.completedSourcePaymentsUsd : "Not loaded"} note={snapshot.hasSourceRecords ? `${foreignCurrencyCount} foreign-currency record${foreignCurrencyCount === 1 ? "" : "s"} retained separately; never a financial total` : "No B2C source records have been received yet"} tone="neutral" />
          ...
          <SectionCard title="B2C reporting reconciliation" description="This bridge explains exactly why source activity and reportable cash differ. Foreign-currency source records remain visible in their original currency and are excluded from USD cards until Finance approves a documented conversion." className="mt-4">
            ...
            {!snapshot.hasSourceRecords ? (
              ...
              <>
                ...
                {visibleRows.length === 0 ? (
                  ...
                  <LedgerTable rows={visibleRows} sort={sort} onSort={changeSort} />
                )}
              </>
            )}
          </SectionCard>
