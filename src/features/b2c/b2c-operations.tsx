...
export function B2cOperations({ snapshot = null, loadError }: { snapshot?: B2cDashboardSnapshot | null; loadError?: string }) {
  ...
  return <AppShell title="B2C operations" description="Stripe and other B2C payments are kept separate from B2B. Only completed, verified, non-duplicate payments contribute to totals; a Finance exception is clearly labelled and fully audited. Refunds remain separate source records." controls={snapshot ? <B2cPeriodSelector month={snapshot.period.month} /> : undefined}>
    {loadError || !snapshot ? <ErrorState title="B2C data unavailable" description={loadError ?? "The B2C dashboard could not be loaded."} /> : <>
      ...
      <SectionCard title="B2C reporting reconciliation" description="This bridge explains exactly why source activity and reportable cash differ. A completed source payment is counted only after verification and duplicate checks, unless an Admin records a clearly labelled, audited Finance exception." className="mt-4">
