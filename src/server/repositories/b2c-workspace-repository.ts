...
export type B2cWorkspaceCounts = { all: number; data: number; duplicates: number; reconciliation: number };
...
/** Summarizes the internally detailed work items into the visible Work queue filter counts. */
export function summarizeB2cWorkItemCounts(items: B2cWorkItem[]): B2cWorkspaceCounts {
  const counts: B2cWorkspaceCounts = { all: items.length, data: 0, duplicates: 0, reconciliation: 0 };
...
export class SupabaseB2cWorkspaceRepository {
  ...
  async overview(today = new Date()): Promise<B2cWorkspaceOverview> {
    const [snapshot, sourceFailures] = await Promise.all([
